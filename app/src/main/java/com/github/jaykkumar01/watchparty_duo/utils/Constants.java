package com.github.jaykkumar01.watchparty_duo.utils;

public class Constants {
    // Notification Channel Details
    public static final String CHANNEL_ID = "toggle_channel";
    public static final String CHANNEL_NAME = "Toggle Channel";
    public static final String CHANNEL_DESCRIPTION = "Channel for toggle notifications";

    // Notification IDs
    public static final int NOTIFICATION_ID = 2;
    public static final int NOTIFICATION_PERMISSION_CODE = 100;

    // Permission Codes
    public static final int MIC_PERMISSION_CODE = 200;
    public static final int CAMERA_PERMISSION_CODE = 300;

    // Extras
    public static final String NAME = "name";
    public static final String PEER_ID = "peerId";
    public static final String REMOTE_ID = "remoteId";
    public static final CharSequence NOTIFICATION_DESCRIPTION_NOT_CONNECTED = "Waiting for connection...";
    public static final CharSequence NOTIFICATION_DESCRIPTION_CONNECTED = "Connection Established";
    public static final String PEER = "peer";
}
